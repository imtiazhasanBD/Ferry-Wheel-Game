...
// import { useEffect, useMemo, useRef, useState, useCallback } from "react";
// import { motion, AnimatePresence, useAnimation, useReducedMotion } from "framer-motion";
// import { X, Volume2, ScrollText, MessageCircleQuestionMark } from "lucide-react";
// import { useGame, type ApiHistoryItem } from "./useGame.socket";
// import { FOODS, type FoodsKey } from "./types";
// import LeaderboardModal from "./LeaderboardModal";
// import GameRules from "./GameRules";
...
// const WINNER_POPUP_DELAY_MS = 700; // hold on winner before showing popup
...
// export default function App() {
//     const game = useGame() as any;
//     const { user, round, placeBet, echoQueue, shiftEcho, creditWin, updateBalance, balance, getRoundWinners, getCurrentHistory, myBetTotal, setting, ping } = game;
//     const startNextRound: (() => Promise<void> | void) | undefined =
//         game.startNextRound || game.startNextBlock || game.nextRound || game.startRound;
//     const prefersReducedMotion = useReducedMotion();
//     const [showRoundWinners, setShowRoundWinners] = useState(false);
//     const [roundWinners, setRoundWinners] = useState<TopWinnersResponse | null>(null);
...
//     const HL_COLORS = ["#22c55e", "#3b82f6", "#ef4444", "#a855f7", "#06b6d4", "#f59e0b", "#10b981"];
...
//         let timer: number;
//         const hop = () => {
//             setPlatIdx(prev => (prev === null ? 0 : (prev + 1) % PLATFORM_KEYS.length));
...
//             try {
//                 const data = await getRoundWinners();
//                 console.log("roundddddddddddddddddddddddddddddddddd winnerrrrrrrrrrrrr", data) // no arg uses current round id
//                 if (alive) setRoundWinners(data);
//             } catch (err) {
//                 console.error(err);
//             }
...
//     console.log("roundddddddddddddddddddd", progress, token)
...
//     console.log("winHistoryyyyyyyyyyyyyyyyyyyyyyyyyy", winnersHistory)
...
//                                             borderWidth:
//                                                 (platIdx === 0) ||
//                                                     ((round?.roundStatus === "revealed" || round?.roundStatus === "completed") && round?.winnerBox === "Pizza")
//                                                     ? 6 : 2,
//                                             borderColor:
//                                                 ((round?.roundStatus === "revealed" || round?.roundStatus === "completed") && round?.winnerBox === "Pizza")
//                                                     ? "#22c55e" // winner = green
//                                                     : (platIdx === 0 ? platColor : "#f97316"), // active hop or default orange-500
//                                             boxShadow:
//                                                 (platIdx === 0) ||
//                                                     ((round?.roundStatus === "revealed" || round?.roundStatus === "completed") && round?.winnerBox === "Pizza")
//                                                     ? `0 0 0 3px ${(platIdx === 0 ? platColor : "#22c55e")}55,
//            0 0 18px ${(platIdx === 0 ? platColor : "#22c55e")}66,
//            0 10px 24px rgba(0,0,0,.45)`
//                                                     : "0 2px 4px rgba(0,0,0,.35)",
//                                             transition: "border-color 60ms linear, border-width 60ms linear, box-shadow 60ms linear",
//                                         }}
//                                     >
//                                         <span className="text-3xl pb-1">🍕</span>
//                                     </div>
...
//                                             borderWidth:
//                                                 (platIdx === 1) ||
//                                                     ((round?.roundStatus === "revealed" || round?.roundStatus === "completed") && round?.winnerBox === "Salad")
//                                                     ? 6 : 2,
//                                             borderColor:
//                                                 ((round?.roundStatus === "revealed" || round?.roundStatus === "completed") && round?.winnerBox === "Salad")
//                                                     ? "#22c55e"
//                                                     : (platIdx === 1 ? platColor : "#16a34a"), // default green-600
//                                             boxShadow:
//                                                 (platIdx === 1) ||
//                                                     ((round?.roundStatus === "revealed" || round?.roundStatus === "completed") && round?.winnerBox === "Salad")
//                                                     ? `0 0 0 3px ${(platIdx === 1 ? platColor : "#22c55e")}55,
...
//                                     <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
//                                         {(winnersHistory.length ? [...winnersHistory] : []).map((k, idx) => {
//                                             const boxKey = (norm((k as any).winningBox ?? (k as any).title) as FoodsKey);
//                                             const emoji = EMOJI[boxKey] ?? "❓";
//                                             const labelText = LABEL[boxKey] ?? ((k as any).box ?? (k as any).title ?? "");
//                                             return (
//                                                 <div key={`${k}-bar-${idx}-${round?.roundId ?? "r"}`} className="relative shrink-0">
...
//                                                         title={labelText}
//                                                     >
//                                                         <span className="text-[16px] leading-none">{k.winningBox === "Pizza" ? "🍕" : k.winningBox === "Salad" ? "🥗" : emoji}</span>
//                                                     </div>
//                                                     {idx === 0 && (
//                                                         <div
//                                                             className="absolute left-1/2 -translate-x-1/2 -bottom-1 px-1.5 py-[1px] rounded-full text-[6px] font-black"
...
//                             </div>
//                             <div className="px-2 rounded-full bg-blue-400/40 backdrop-blur-md border border-white/20 shadow">
//                                 Total {fmt(round.boxStats?.reduce((total: any, box: any) => total + box.totalAmount, 0) || 0)}
//                             </div>
//                         </div>
...
//                             </div>
//                             <div className="rounded-lg p-1.5 text-white/95 border border-white/20 bg-black/15 flex flex-col items-center">
//                                 <div className="text-[11px] opacity-90 leading-none">Today's Win</div>
//                                 <div className="text-[14px] font-bold tabular-nums leading-tight">
//                                     💎 {fmt((user?.profit ?? 0) - (user?.loss ?? 0))}
//                                 </div>
//                             </div>
          ...
          `,
                    ...
                    <div className="px-3 mt-3">
                        ...
                        >
                            ...
                            <div onClick={() => setShowTodayLeaderboard(true)} className="rounded-lg p-1.5 text-white/95 border border-white/20 bg-black/15 flex flex-col items-center">
                                <div className="text-[11px] opacity-90 leading-none">Today's Win</div>
                                <div className="text-[14px] font-bold tabular-nums leading-tight">
                                    💎 {fmt((user?.profit ?? 0) - (user?.loss ?? 0))}
                                </div>
                    ...
                    <TodayLeaderboardModal
                        ...
                        onClose={() => { setShowTodayLeaderboard(false) }}
                    // onStartNow={() => setIntermissionSec(0)}
                    />
                    <GameRules open={showGameRules} onClose={() => setShowGameRules(false)} />
                    <Record open={showRecord} onClose={() => setShowRecord(false)} />
