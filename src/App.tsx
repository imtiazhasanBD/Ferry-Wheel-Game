...
//               // tangent unit vector (left-right orientation along the ring)
//               const tx = -Math.sin(rad);
//               const ty = Math.cos(rad);

//               // Stud settings (left of the button)
//               const studRadius = 5;
//               const gap = 10;
//               const studOffset = btn / 2 + gap;

//               const lx = cx - tx * studOffset;
//               const ly = cy - ty * studOffset;

//               // highlight strictly by forcedWinner
...
//                       isWinner ? "animate-[blink_900ms_steps(2)_infinite] glow" : ""
//                     }`}
//                     style={{
//                       left: cx,
//                       top: cy,
...
//                     disabled={disabled}
//                   >
//                     {/* counter-rotated inner content keeps emoji/text upright */}
//                     <div
...
//                       {/* count badge */}
//                       <div
//                         className="absolute -left-1 -top-1 rounded-full px-2 py-0.5 text-[10px] text-white tabular-nums shadow"
//                         style={{
//                           background: "linear-gradient(180deg,#60a5fa,#2563eb)",
//                           boxShadow: "0 6px 14px rgba(0,0,0,.45)",
//                           border: "1px solid rgba(255,255,255,.25)",
...
//                 {/* Result tiles (newest on the left) */}
...
//                       {/* NEW badge for the most recent (left-most) */}
//                       {idx === 0 && (
//                         <div
...
//       onClick={onClick}
//       className="w-10 h-10 rounded-xl bg-white/10 border border-white/20 shadow grid place-items-center text-[18px] hover:bg-white/20 active:scale-95 text-white"
//       style={{ boxShadow: "0 8px 18px rgba(0,0,0,.4), inset 0 1px 0 rgba(255,255,255,.2)" }}
//     >
//       {children}
//     </button>
//   );
...
//         <span className="tabular-nums">{Math.round(value * 100)}%</span>
//       </div>
//       <input type="range" min={0} max={1} step={0.01} value={value} onChange={(e) => onChange(parseFloat((e.target as HTMLInputElement).value))} className="w-full accent-[#22c55e]" />
//     </div>
//   );
// }
...
//   const { user, round, placeBet, echoQueue, shiftEcho } = game;
...
//       const idx = indexUnderPointer(total, 360 / FRUITS.length, FRUITS.length);
//       const ideal = rotationToCenterIndex(idx, 360 / FRUITS.length);
//       const k = Math.round((total - ideal) / 360);
...
//   const onSliceClick = useCallback(
//     async (key: FruitKey) => {
//       // Disable bet if the round is over
//       if (!bettingOpen || isRoundOver || !selectedChip) return;

//       // Update the user's total bet for this fruit
...
//       await placeBet(key, selectedChip);
//     },
//     [bettingOpen, isRoundOver, selectedChip, placeBet]
//   );
...
//   function handleWin(winner: FruitKey) {
...
//               <button
//                 aria-label="Exit"
//                 className="text-white text-lg p-1 rounded-full border border-white/20 bg-white/20 transition-all duration-300"
//                 onClick={() => {/* Implement your exit logic here */ }}
//               >
//                 ↵
//               </button>
...
//             {/* Right Side: Sound, Help, Ping & Wi-Fi */}
//             <div className="flex items-center space-x-2 justify-end">
//               {/* Sound Icon */}
//               <button
//                 aria-label="Sound Settings"
//                 className="text-white text-lg p-1 rounded-full border border-white/20 bg-white/20 transition-all duration-300"
//                 onClick={() => setSettingsOpen(true)} // Assuming this opens the settings
//               >
//                 ♫
//               </button>
...
//             {/* Left Side: Block Round and Today's Round */}
//             <div className="text-white text-xs opacity-80">
//               <div>Today's Round: {roundNum}</div>
//               <div className="font-bold">Round: #{displayBlockRound}</div>
//             </div>

//             {/* Right Side: Cup Icon (Leaderboard) */}
//             <div className="flex justify-end items-center gap-2">
//               <div className="flex items-center space-x-1 text-green-500">
//                 <span className=" text-lg">ᯤ</span> {/* Wi-Fi Icon */}
//                 {/* Ping Indicator */}
//                 <span className=" text-xs">45ms</span> {/* Example Ping */}
//               </div>
//               <button
//                 aria-label="Leaderboard"
//                 className="text-white text-lg p-1 rounded-full border border-white/20 bg-white/20 transition-all duration-300"
//                 onClick={() => setShowLeaderboard(true)} // Assuming this opens the leaderboard
//               >
//                 🏆
//               </button>
//             </div>
...
//               const angDeg = i * (360 / FRUITS.length);
//               const rad = ((angDeg - 90) * Math.PI) / 180;
//               const cx = R + ringR * Math.cos(rad);  // X-coordinate for the slice button
//               const cy = R + ringR * Math.sin(rad);  // Y-coordinate for the slice button

//               const totalBet = userBets[key];  // Get total bet for the current fruit slice
//               const studRadius = 5;  // Adjust stud size as needed
//               const studOffset = btn / 2 + 10;  // Offset for stud positioning

//               // Calculate the stud position based on the angle and ring radius
//               const tx = -Math.sin(rad);  // X-offset for stud
//               const ty = Math.cos(rad);  // Y-offset for stud

//               // Position of the stud on the wheel
//               const lx = cx - tx * studOffset;
//               const ly = cy - ty * studOffset;

//               // Define the disabled state for the button (e.g., disable when not in betting state)
//               const disabled = round?.state !== "betting" || showLeaderboard;

//               // Define if this slice is the winner (e.g., highlight the winning slice)
//               const isWinner = forcedWinner === key && round?.state !== "betting";
...
//                     whileTap={{ scale: 0.96 }}
//                     whileHover={prefersReducedMotion ? {} : { translateZ: 10 }}
//                     onClick={() => !disabled && onSliceClick(key)}  // Prevent click if disabled
...
//         {/* transient & payout flies (unchanged) */}
//         <div className="pointer-events-none absolute inset-0 z-30">
//           <AnimatePresence>
//             {flies.map((f) => (
//               <motion.div
...
//                 animate={{ x: f.to.x - 2, y: f.to.y - 1, opacity: 1, scale: 1, rotate: 360 }}
//                 exit={{ opacity: 0, scale: 0.7 }}
//                 transition={{ type: "spring", stiffness: 220, damping: 22 }}
//                 className="absolute w-10 h-10"
//               >
...
//                 key={f.id}
//                 initial={{ x: f.from.x - 1, y: f.from.y - 1, opacity: 0, scale: 0.85 }}
//                 animate={{ x: f.to.x - 1, y: f.to.y - 1, opacity: 1, scale: 1, rotate: 360 }}
//                 exit={{ opacity: 0, scale: 0.7 }}
//                 transition={{ type: "spring", stiffness: 220, damping: 22 }}
//                 className="absolute w-10 h-10"
//               >
...
//           open={showLeaderboard}
//           onClose={() => {
//             // optional: allow manual close (does NOT auto-start)
//             setShowLeaderboard(false);
//             setIntermissionSec(undefined);
//           }}
//           onStartNow={() => {
//             // skip the wait: start now + reset block
...
//       <Record open={showRecord} onClose={() => setShowRecord(false)} />      {/* Settings bottom sheet */}
//         <AnimatePresence>
//           {settingsOpen && (
//             <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", stiffness: 260, damping: 26 }} className="fixed inset-x-0 bottom-0 z-50">
//               <div className="mx-auto w-[350px] max-w-[350px] rounded-t-2xl bg-white/10 text-white shadow-2xl border-t border-white/15 p-4 backdrop-blur-xl">
...
// function Coin() {
//   return (
//     <div className="w-5 h-5 rounded-full border shadow-2xl flex items-center justify-center"
//       style={{
//         background: "radial-gradient(circle at 40% 30%, #fde68a, #fbbf24 55%, #f59e0b)",
//         borderColor: "#fbbf24",
//         boxShadow: "0 14px 30px rgba(0,0,0,.5), inset 0 2px 0 rgba(255,255,255,.45)"
...
//               <button
//                 aria-label="Record"
//                 className="p-2 rounded-full bg-white/10 border border-white/20 text-yellow-300 hover:bg-white/20 transition"
//                 onClick={() => setShowRecord(true)}
//               >
//                 <ScrollText size={18} />
//               </button>
...
//                 </div>
//                 <div className="bg-[#0864b4] text-white text-[8px] font-bold p-1 rounded-md -mt-2 shadow absolute z-30">
//                   Total 400k
//                 </div>
//                 <div className="bg-[#0864b4] text-white text-[8px] font-bold p-1 rounded-md -ml-3 -bottom-2 shadow absolute z-30">
//                   3.45x
//                 </div>
...
//                   <span className="text-3xl">🥗</span>
//                 </div>
//                 <div className="bg-[#0864b4] text-white text-[8px] font-bold p-1 rounded-md -mt-2 shadow absolute z">
//                   Salad
//                 </div>
//                 <div className="bg-[#0864b4] text-white text-[8px] font-bold p-1 rounded-md -mr-3 -bottom-2 shadow absolute z-30">
//                   3.45x
//                 </div>
...
//                   />
//                 </svg>
//                 <div className="absolute right-3/12 mt-5 mr-2">
      ...
      >
       ...
       C 285 55, 330 85, 360 110"
        ...
        <div className="px-3 relative">
          <div className="absolute left-4 right-4 -top-7 flex justify-between text-white text-[12px]">
            <div className="px-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 shadow">
              Mine 200k
            </div>
            <div className="px-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 shadow">
              Total 100M
              ...
              <motion.div
                ...
                animate={{
                  x:
                    ...
                    (phoneRef.current?.getBoundingClientRect()?.left ?? 0) +
                    (bankRef.current?.getBoundingClientRect()?.width ?? 40) / 2,
                  y:
                    (bankRef.current?.getBoundingClientRect()?.top ?? 0) -
                    (phoneRef.current?.getBoundingClientRect()?.top ?? 0) +
                    (bankRef.current?.getBoundingClientRect()?.height ?? 40) / 2,
                  opacity: 1,
                  scale: 1,
                  rotate: 360,
                }}
